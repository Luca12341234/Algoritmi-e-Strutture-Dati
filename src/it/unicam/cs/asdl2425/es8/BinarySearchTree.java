 ...
 * elementi presenti.
 * 
 * @param E il tipo delle etichette dei nodi in questo Binary Search Tree. La
 *          classe {@code E} deve avere un ordinamento naturale definito tra gli
 *          elementi.
 * 
...
public class BinarySearchTree<E extends Comparable<E>> {
	...
	/**
	 * Restituisce l'altezza di questo albero. L'altezza è definita come la massima
	 * lunghezza di un percorso dal nodo radice a un nodo foglia in questo albero.
	 * L'altezza dell'albero vuoto è -1, l'altezza dell'albero con una radice/foglia
	 * è 0, e così via.
	 * 
	 ...
	 * 
	 * @param l una lista (può essere anche vuota) su cui inserire le etichette in
	 *          ordine.
	 * @throws NullPointerException se la lista passata è null
	 */
	public void addOrderedLabelsTo(List<E> l) {
		if (l == null)
			throw new NullPointerException("Lista null su cui aggiungere le etichette");
		if (this.isEmpty())
			return;
	 ...
	 * 
	 * @return la lista ordinata delle etichette dei nodi di questo albero secondo
	 *         l'ordinamento naturale della classe {@code E}
	 */
	...
	/**
	 * Restituisce l'etichetta più piccola, in base all'ordinamento naturale della
	 * classe {@code E}, presente nell'albero.
	 * 
	...
	/**
	 * Restituisce l'etichetta più grande, in base all'ordinamento naturale della
	 * classe {@code E}, presente nell'albero.
	 * 
	 * @return l'etichetta massima presente nell'albero oppure null se l'albero è
	 *         vuoto
	 */
	public E getMax() {
		if (this.isEmpty())
	 ...
	 * 
	 * @return l'etichetta successore di {@code label} in questo albero, oppure null
	 *         se {@code label} non ha un successore
	 * @throws IllegalArgumentException se l'etichetta {@code label} non è presente
	 *                                  in questo albero
	 * @throws NullPointerException     se l'etichetta passata è null
	 */
	public E getSuccessor(E label) {
		if (label == null)
			throw new NullPointerException("Etichetta di cui cercare il successore null");
		if (this.isEmpty())
	 ...
	 * 
	 * @return l'etichetta predecessore di {@code label} in questo albero, oppure
	 *         null se {@code label} non ha un predecessore
	 * @throws IllegalArgumentException se l'etichetta {@code label} non è presente
	 *                                  in questo albero
	 * @throws NullPointerException     se l'etichetta passata è null
	 */
	public E getPredecessor(E label) {
		if (label == null)
			throw new NullPointerException("Etichetta di cui cercare il predecessore null");
		if (this.isEmpty())
	 ...
	 * 
	 * @return true se il nodo è stato effettivamente inserito, false se l'etichetta
	 *         era già presente
	 * @throws NullPointerException se l'etichetta passata è null
	 */
	...
	public boolean remove(E label) {
		if (label == null)
			throw new NullPointerException("Tentativo di rimuovere etichetta null");
		if (this.isEmpty())
	 ...
	 * 
	 * @return the RecBST node corresponding to the root of this binary search tree.
	 */
	...
	protected class RecBST {
		...
		/*
		 * Genitore di questo (sotto-)albero, può essere null quando questo nodo è la
		 * radice dell'albero rappresentato dall'oggetto della classe principale.
		 */
		...
		/*
		 * Costruisce un (sotto-albero) a partire da un nodo, due sotto-alberi e un nodo
		 * genitore.
		 * 
		 * @param label etichetta da associare al nodo
		 * 
		...
		protected int computeHeight() {
			...
			int l = 0, r = 0;
			if (this.left != null)
				l = this.left.computeHeight() + 1;
			if (this.right != null)
				r = this.right.computeHeight() + 1;
			return Math.max(l, r);
		}
		...
		protected boolean insert(E label) {
			// TODO implementare ricorsivamente
			int num = label.compareTo(this.label);
			if (num < 0) {
				if (this.left == null) {
		 ...
		 * @param label l'etichetta da cercare
		 * 
		 * @return il puntatore al nodo che contiene l'etichetta cercata, oppure null se
		 * l'etichetta non è presente
		 */
		protected RecBST search(E label) {
			// TODO implementare ricorsivamente
			if (label.compareTo(this.getMaxNode().label) > 0 || label.compareTo(this.getMinNode().label) < 0)
				return null;
			...
			if (this.label.equals(label))
				...
				this.left.search(label);
			else
				this.right.search(label);
			return null;
		}

		/*
		 * Aggiunge ad una lista data le etichette dei nodi di questo (sotto-)albero
		 * nell'ordine naturale. Per far questo esegue una visita in-order di questo
		 * (sotto-)albero.
		 * 
		 * @param l una lista (può essere anche vuota) su cui inserire le etichette in
		 * ordine
		 ...
		 * (sotto-)albero secondo l'ordinamento naturale della classe {@code E}. Per
		 * ottenere il risultato fa una visita in-order.
		 * 
		 * @return la lista ordinata delle etichette dei nodi di questo (sotto-)albero
		 * secondo l'ordinamento naturale della classe {@code E}
		 */
		 ...
		 * 
		 * @return il nodo più a sinistra che non ha un sotto-albero sinistro in questo
		 * (sotto-)albero
		 */
		protected RecBST getMinNode() {
			// TODO implementare ricorsivamente
			return this.left!=null ? this.left.getMinNode() : this;
		}

		/*
		 * Restituisce il puntatore al nodo che contiene l'etichetta più grande presente
		 * in questo (sotto-)albero.
		 * 
		 * @return il nodo più a destra che non ha un sottoalbero destro in questo
		 * (sotto-)albero
		 */
		protected RecBST getMaxNode() {
			// TODO implementare ricorsivamente
			return this.right!=null ? this.right.getMaxNode() : this;
		}
		 ...
		 * 
		 * @return il puntatore al nodo successore oppure null se questo nodo non ha
		 * successore
		 */
		protected RecBST getSuccessorNode() {
            ...
            RecBST successor = null,current = this;
        ...
        protected RecBST getPredecessorNode() {
        	...
        	if (this.left != null) 
                return getMaxNode();  
            RecBST predecessor = null, current = this;
            while (current.parent != null) {
        ...
        protected void deleteSelfLabel() {
            ...
            // Caso 2: Nodo con un solo figlio
                ...
                // Nodo ha solo un figlio destro
                if (this.parent == null) {
                    // Se è la radice, il figlio diventa la nuova radice
                    this.label = this.right.label;
                ...
                // Nodo ha solo un figlio sinistro
                if (this.parent == null) {
                    // Se è la radice, il figlio diventa la nuova radice
                    this.label = this.left.label;
            ...
            // Caso 3: Nodo con due figli
                ...
                // Troviamo il successore (minimo del sottoalbero destro)
                RecBST successor = this.right.getMinNode();
                this.label = successor.label;
                successor.deleteSelfLabel(); // Elimina il successore, che sarà nel caso 1 o 2
            }
        }
	}
}
